...
```json
{
  ...
  "request_id": "uuid4",
  "verb": "health | inventory | repo.probe | execute | worktree.prepare | worktree.prepare_status | worktree.verify | start | status | cancel | spool.release | worktree.finalize | worktree.push",
...
|---|---|---|---|
| `health` | `{}` | `ollama: {reachable, base_url, error}`, `codex: {available, detail}`, `gpu_yield: {active, reason}`, `in_flight: [execution_id]`, `write_prerequisites: {decision_fs, runner_units, detail}` (Stage 6, S6.11/S6.12), `time_utc` | `_codex_available`, `experiment_priority_active` (evaluated on the worker host) |
...
| `worktree.prepare` | `{repo_id, branch, base_ref, lease: {lease_id, host_id}}`. `lease` is required from Stage 6 (§6.0 S6.7); if missing → `bad_request` before any git mutation | `{state: prepared \| prepare_failed, path, branch, head_sha, clean, reused, error?}`. A durable per-`lease_id` record is claimed atomically before mutation. After the claim the verb always returns `ok: true`, and a same-`lease_id` call returns the record (S6.9) | `worktree_ops.create_or_reuse_worktree`, `verify_worktree`, `git_is_clean` |
| `worktree.prepare_status` | `{lease_id, fence: bool}` (Stage 6, S6.9) | `{state: unknown\|fenced\|preparing\|prepared\|prepare_failed\|prepare_interrupted, process_alive, record}`. `fence: true` atomically claims an absent record as `fenced` | prepare record + `estate_worker_procs.tree_quiescent` (S6.12) |
| `spool.release` | `{execution_id, resolution: not_started\|finalized\|recovered}` (Stage 6, S6.8) | `{released, state, already_released}`. Refused for a live/`starting` writer. The spool then compacts to a permanent tombstone after 7 days | spool |
| `worktree.push` | `{execution_id, repo_id, branch, commit_sha}` (`execution_id` keys the per-execution push-attempt records, round-7 finding) (Stage 6, S6.10) | `{pushed, contained, commit_sha, error?}`. Exact-commit push, never forced; `not_found` if the commit is absent | repo path from `resolve_repo_path` |
...
| `start` | `{execution_id, kind: "codex-write", objective, repo_id, lease: {lease_id, worktree_path, branch, expected_head_sha}, timeout_s}` (`expected_head_sha` added in Stage 6, S6.10) | `{accepted, state, handle: {pid, create_time, spool_id} \| null, reused: bool, spawned?}` — `state` per the S6.6 start state machine | re-runs `worktree.verify` (TOCTOU), atomically claims the spool with a durable `starting` state (S6.6), then spawns detached `python -m src.estate_worker --run-spooled <execution_id>`, which calls `_execute_codex_with_sandbox(sandbox="workspace-write", cwd=verified path, on_started=spool writer)` |
| `status` | `{execution_id, fence?: bool, close?: bool}` (`fence` added in Stage 6, S6.8; `close` in Stage 6, S6.12 execution closure) | `{state: unknown\|fenced\|tombstone\|starting\|running\|succeeded\|failed\|timed_out\|start_failed, handle, quiescent: true\|false\|null, process_alive (= quiescent is not true), spawned, started_at, finished_at, released?, result?}`, where `quiescent` is S6.12 tree quiescence, never single-pid liveness. `starting`/`start_failed`/`spawned` were added in Stage 6 (S6.6), and `fenced`/`tombstone`/`released` also in Stage 6 (S6.8) | spool decision files + `estate_worker_procs.tree_quiescent(handle)` (S6.12) |
| `cancel` | `{execution_id}` | `{killed, still_alive_pids}` | `estate_worker_procs.kill_tree(handle)` |
| `worktree.finalize` | `{execution_id, repo_id, worktree_path, branch, commit_message, expected_head_sha}` (`execution_id`/`expected_head_sha` added in Stage 6, S6.10) | `{committed, adopted, commit_sha, parent_sha, dirty_paths, push: {state, error?}}` or `finalize_ambiguous`. It commits only from HEAD == `expected_head_sha`, adopts only a HEAD proven by its own `commit.json`, and makes one exact-commit push attempt (S6.10) | git logic moved out of `finalize_execution` (`:1434-1476`) |
...
- The spool lives at `~/.aoteru/worker-spool/<execution_id>/` and holds `request.json`, `state.json` and `result.json`. It is evidence for the control plane, **not** a lifecycle authority. (S6.8) Spool directories are never deleted. A write spool compacts to a permanent tombstone only 7 days after the control plane's `spool.release` acknowledgement. A self-test spool compacts 7 days after it becomes terminal. Terminal, fenced and tombstoned `execution_id`s are permanently unstartable.
...
| Push state | `estate_executions.finalization_json.push` (Stage 6, S6.10) | Control plane only | `not_required \| pushed \| failed`. It is separate from `worktree_resolution`: `finalized` never implies pushed, and a `failed` push has the governed retry `push_finalized_execution` |
...
- New column `EstateExecution.worktree_resolution` (String, NOT NULL, default `unresolved`). Admission inserts every row as `unresolved`. Only three transitions leave `unresolved`, and each commits inside the lease-serialized transaction of S6.5:
  ...
  - `finalized`: only in `finalize_execution`. Preconditions: the row is `succeeded`, the lease is still authoritative with the identical `lease_id`/`host_id`/`worktree_path`/`branch`, the worker `worktree.finalize` returned success under the S6.10 history bound (`expected_head_sha = admission_head_sha`), and a post-finalize `worktree.verify` reports `clean: true` for that exact path and branch. `finalized` means committed locally and clean, **not** pushed (S6.10). A failed push is recorded as `finalization_json.push.state = "failed"`, with the governed retry `push_finalized_execution` and a `next_action`. It does not keep the worktree unresolved, because the commit exists on the branch and the tree is clean.
...
- Preconditions. Any failure → refused, and nothing changes:
  ...
  4. A **fresh** worker `status` on `host_id` (not a cached observation) answers, its handle matches `worker_handle_json`, and it reports a terminal or `start_failed`/`fenced` state with **aggregate `quiescent: true`** (S6.12). That covers the writer runner **and every finalize attempt unit** of the execution. Push attempts are excluded, per S6.12, because they never touch the worktree. so recovery in the S6.10 crash window waits for, or fences, an in-flight finalize attempt. The call is `status {close: true}` (S6.12 execution closure), so an earlier-authorised finalize that reaches the worker after this point refuses `execution_closed` before any git command. `process_alive: false` from a single pid is no longer enough, and `quiescent: null` → refused `writer_quiescence_unproven`. Worker unreachable → `worker_unreachable`, refused. Anything the control plane cannot observe, it cannot recover.
  5. A **fresh** worker `worktree.verify` for the exact path and branch returns `ok: true` and `clean: true`. If HEAD is a `commit.json`-proven finalize commit for this execution, recovery **does not refuse** (round-9 finding: after closure, finalize can no longer run, so a refusal would deadlock). It records `recovery = {head_sha: commit.json.commit_sha, finalize_commit: true, …}` and `push = {state: "not_attempted", commit_sha}`, so the S6.10 governed push applies to that commit. Dirty → `worktree_not_clean` with the evidence. The operator resets, commits or stashes, then retries. Operator commits are allowed. The resulting `head_sha` is recorded.
...
- After the commit: best-effort `spool.release {resolution: recovered}` (S6.8). The same happens after every `not_started` write.
...
- New `park_lease_ops.lease_authority_state(db, lease, *, now=None) -> {lease_id, status, heartbeat_stale, protected_by_execution_id, authoritative, reclaimable, reason}`:
  ...
  - `preparing` → a reservation. It blocks competing parks and routing to other hosts, but is **not** write authority (`authoritative: False` for admission, finalize and recovery). (S6.9) `reclaimable` is **always** `False`. An aged `heartbeat_at` (older than `PARK_PREPARE_STALE_SECONDS = 300`) only makes the reservation eligible for the `resolve_preparing_reservation` probe, which releases it once the worker proves the original prepare cannot still be running. A `preparing` row can never have executions.
...
**S6.6 Race-safe same-`execution_id` `start` (worker).** (Rewritten by S6.11/S6.12 after the round-0 adjudication of PR #575.) The earlier temp-directory-then-`os.rename` claim is withdrawn. POSIX `rename(2)` silently replaces an *empty* target directory, and a compare-then-overwrite of `state.json` is not atomic. Every decision below goes through the S6.11 `decide_once` primitive instead.
- *Spool layout.* `<spool_root>/<execution_id>/` holds these files:
  - `claim.json`, `decide_once`. It is `{kind: "start", request, claimed_at}` or `{kind: "fence", fenced_at}`, and carries the full request, so no separate mutable `request.json` is authoritative.
  - `run.json`, `decide_once`. It is `{decision: "execute", pid, create_time, at}`, written by the runner, or `{decision: "abort", by: "starter" | "observer" | "fence", reason, at}`.
  - `released.json`, `decide_once` (S6.8).
  - Mutable telemetry: `state.json`, replaced via `os.replace` and never read as a decision. It holds the starter's spawn handle, `running`/terminal progress, and the writer's own session handle (S6.12).
  - `result.json` and `worker.log`.

  A directory with no `claim.json` is unclaimed, a harmless pre-claim leftover. Claims are never inferred from directory existence.
- *Claim.* `_verb_start` validates its payload, including the S6.10 `expected_head_sha` TOCTOU re-verify, then calls `decide_once(claim.json, {kind: "start", request})`. It spawns **only if it won**. A loser answers from the existing files, as below.
- *Errors mean pre-claim, and only that* (Stage 6 gate, round 8). After a `start` or `prepare` claim, the verb never returns an error; every post-claim failure is answered from the decision files. The control plane takes a worker error as proof of `not_started` only for the pre-claim codes `bad_request`, `unsupported_protocol`, `unknown_verb`, `identity_*`, `executor_unavailable` and `authority_denied`, and only under the sole-first-attempt rule. Any other code, including `execution_failed`, is ambiguous and resolves through the fence. The same code allowlist applies to `worktree.prepare` reservations.
- *Runner.* `_run_spooled` starts with `decide_once(run.json, {decision: "execute", pid: own, create_time: own})`. If it loses, an abort already won; it exits without executing anything. The runner never waits for the starter's handle, so the old 5 s handle wait and its `failed`-on-timeout disappear.
- *Deterministic spawn failure.* On `ProcessLayerError`/`OSError` before a child exists, the starter calls `decide_once(run.json, {decision: "abort", by: "starter", error})`, then records `result.json`. Nothing is deleted.
- *Stale or fenced `starting`.* A `start` claim with no `run.json` is `starting`. The worker writes an abort, `decide_once(run.json, {decision: "abort", by: "observer" | "fence"})`, only when:
  - (a) `status` is called with `fence: true`, whatever the claim's age (S6.8); or
  - (b) the claim is older than `STARTING_STALE_SECONDS = 60` and `status`/`start` touches it.

  If the abort wins, the writer can never execute, because any runner loses its own `run.json` decision, so `start_failed` is positively proven. If the abort loses, the runner decided `execute` first and its real state is reported. **Age alone never produces `start_failed` without winning that decision**, which closes the stale-observer overwrite race.
- *Derived process state.* The worker reads only the `decide_once` files and quiescence, never an overwritable field:
  - no claim → `unknown`;
  - fence claim → `fenced`;
  - start claim with no run decision → `starting`;
  - `abort` → `start_failed`;
  - `execute` → the terminal state from `state.json`/`result.json` **only when `tree_quiescent(...) is True`** (round-2 new finding 1). A terminal record with a populated or unknown scope is still reported as `running`, with `terminal_pending: true`, so no consumer can finalize, recover or release while a descendant may still write. Otherwise it is `running`. `handle` = `run.json`'s runner handle, and `process_alive = (quiescent is not True)` (S6.12).
- Same-ID `start` answers from the existing spool and never raises `execution_failed` for an in-progress claim:
  ...
  - `fenced` / `tombstone` → `{accepted: false, state, reused: true}`, never spawns.
...
- Control-plane mapping in step 6:
  ...
  - `start_failed` or `fenced` → `failed` + `not_started` (S6.1);
  - **Refusals after ambiguity (round-0 finding 3).** A pre-claim refusal of `start` proves `not_started` **only** when it answers the first and only `start` the control plane sent for that `execution_id`. Once any earlier `start` for the id had an ambiguous outcome, that earlier request may still be paused before its claim, and a later refusal says nothing about it. The control plane then resolves only through `status {fence: true}`, taking whichever `decide_once` claim won: `fenced`/`start_failed` → `not_started`; otherwise the real state. The same rule applies to `worktree.prepare` (S6.9). The control plane sends at most one `prepare` per `lease_id`, so its only follow-up is `prepare_status {fence: true}`;
  - `status` returns `unknown` → re-issue `start` with the same `execution_id` only while `now < execution_deadline_at`. This is safe because of the atomic claim. At or after the deadline the control plane calls `status {fence: true}` instead, and `fenced` maps to `failed` + `not_started` (S6.8).
...
- New `park_lease_ops.park_with_worktree(repo_id, host_id, branch, *, prepare) -> dict` replaces the mutate-then-lease order for every park that creates or reuses a worktree:
  ...
  2. **Prepare under that authority.** `prepare(repo_id, branch, lease={lease_id, host_id})`. This is always worker `worktree.prepare`, through `SshTransport` for a remote host and `LocalTransport` for the lab host (S6.9). Its payload requires `lease` or it returns `bad_request` before any git mutation, and it keeps the durable per-`lease_id` prepare record. No lock is held during the call.
  ...
  5. **Failure** (S6.9 classification). Release only on a **positively known** outcome: a worker pre-claim refusal (`ok: false` with a worker-side code), `prepare_failed`, or `prepared` failing the step-3 evidence check (`clean != true`, branch mismatch). In those cases, in one S6.5 transaction, set the reservation `released`; this is safe, because no execution can exist under a non-active lease and no preparation is still running. A refused bind means the reservation is no longer this caller's, so there is nothing to release. **Ambiguous** outcomes leave the reservation `preparing` and return 409 `prepare_outcome_unresolved`, pending `resolve_preparing_reservation`: `worker_unreachable`, `worker_protocol_error`, `placement_mismatch`, timeout, or a lost response. The call returns the existing 409 shapes (`RepoNotClean`, `WorktreeVerificationError`). A worktree the worker may already have created stays on disk as clean, unowned evidence. Automation never deletes it on a remote host. A later park reuses it only through the same sequence, and a dirty reused worktree fails closed at step 3.
- The branchless local park (the live checkout path, no worktree creation) is unchanged: clean check, then `park_repo`.

**Final pre-implementation closure (2026-09-23, obsidian-PhD PR #575).** S6.8–S6.12 close the last three gaps found after `0238bc8`. S6.11/S6.12 and the S6.6 rewrite were added after the round-0 GPT-6 Sol adjudication rejected the first draft (findings 1–6). They take precedence over any older sentence in S6.1–S6.7 or later in this stage. The edited sentences are marked "(S6.8)"–"(S6.12)". Live evidence at `94ad496`:
- `_gc_spools` (`src/estate_worker.py:102-120`) `rmtree`s every spool whose state is terminal and whose `state.json` mtime is older than 7 days, whatever its kind. An unfinalized `succeeded` write row older than that loses the handle that S6.2 precondition 4 requires, and a replayed `start` for the same `execution_id` would then find no spool and spawn.
- `_verb_worktree_prepare` (`:382-401`) keeps no record keyed by lease. A lost response leaves the control plane with no way to tell "never ran" from "still running" from "finished".
- Live `finalize_execution` (`src/estate_router.py:1507-1512`) sets `finalized = pushed`. The worker verb pushes the whole branch (`git push origin <branch>`, `:572`). No operation exists to retry a failed push.

**S6.8 Write-spool retention: claim, fence and tombstone. Nothing ever deletes a spool directory.**
- *Kinds.* A spool whose `request.json` kind is `codex-write` is a **write spool**. `noop-sleep` is a **self-test spool**.
- *No deletion.* The worker never `rmtree`s a spool directory and never deletes or overwrites `claim.json`, `run.json` or `released.json`. Those `decide_once` files are what makes an id unstartable (S6.6/S6.11), not the directory's existence. Compaction deletes only `worker.log` and `result.json`, then `os.replace`s `state.json` with `{state: "tombstone", kind, original_state, resolution, compacted_at}`. It never empties the directory, and nothing depends on non-emptiness anyway. A tombstone is kept forever. It is a few hundred bytes per execution, and every spool-root scan is O(n) over tiny files.
- *When a write spool may compact.* Only when the control plane has acknowledged resolution through the new verb `spool.release`, **and** the release is older than `SPOOL_COMPACT_AFTER_RELEASE_SECONDS = 7 days`, which keeps logs for forensics. A write spool with no release acknowledgement is retained in full indefinitely, whatever its state or age. Heartbeat-free age is never enough.
- *When a self-test spool may compact.* When it is terminal (`succeeded`/`failed`/`timed_out`/`start_failed`) and its `state.json` is older than 7 days. It still becomes a tombstone, never a deletion.
- *Claim leftovers.* A `decide_once` temp file (`.<name>.tmp-*`) older than `STARTING_STALE_SECONDS` is deleted. It was never linked into place, so no reader ever observed it as a decision.
- *`spool.release`* has payload `{execution_id, resolution: not_started | finalized | recovered}` and result `{released: bool, state, already_released: bool}`.
  - It is refused with `authority_denied` if the spool is `starting`, or is `execute`-decided without `tree_quiescent == true` (S6.12). Resolution is never acknowledged for a writer that is live or not proven quiescent.
  - A missing spool → the worker writes a fence tombstone, as in the fence rule below, and returns `released: true`.
  - It is idempotent: `decide_once(released.json, {resolution, released_at})`, and a loser returns `already_released: true`.
  - It is DB-free. It is evidence from the control plane, not a worker decision.
- *Who calls it.* The control plane calls `spool.release` after committing `worktree_resolution ∈ {not_started, finalized, recovered}`, outside any lock. Success sets the new column `EstateExecution.spool_released_at`. Failure or unreachability is logged and is **not** an error for the resolution itself. `reconcile_stale_estate_executions` retries it for every Stage-6 row (non-null `worker_handle_json`) with a resolved `worktree_resolution` and `spool_released_at IS NULL`. Legacy rows and `legacy_closed` rows are never sent.
- *Status fence.* `status` accepts an optional `fence: true`.
  - With no claim, the worker calls `decide_once(claim.json, {kind: "fence"})`.
  - With a `start` claim but no `run.json`, it calls `decide_once(run.json, {decision: "abort", by: "fence"})`.

  Each `decide_once` has exactly one winner. Either the original `start`/runner decided first, and `status` reports its real state, or the fence decided first. In that case a later `start` loses the claim, a later runner loses `run.json`, and the answer is `fenced` or `start_failed`. A `start` on a `fenced` or `tombstone` spool answers `{accepted: false, state: "fenced" | "tombstone", reused: true}` and never spawns. Without `fence`, `status` stays read-only and answers `unknown` for a missing spool.
- *Control-plane mapping.* This amends S6.6 and step 6.
  - `unknown` → re-issue the same-ID `start` only while `now < execution_deadline_at`.
  - At or after the deadline, the control plane never re-issues `start`. It calls `status {fence: true}` instead.
    - `fenced` → proven not started: `failed` + `not_started` (S6.1).
    - A real state → map it as usual.
    - Unreachable → the row stays `accepted`/placeholder, and becomes `lost` after `execution_deadline_at + 120 s` as before.
  - A `lost` placeholder row whose host comes back is resolved the same way. This narrows B9 to a host that **never** returns.
  - A `tombstone` or `released` spool seen for an `unresolved` row is a contradiction, since release follows resolution. The row stays `unresolved`, and the control plane records `error = "spool_released_before_resolution"` for the operator. It never becomes admissible.
- *Invariant.* A writer executes only if its runner won `run.json` with `execute`, and a runner exists only if a `start` won `claim.json`. Neither file is ever deleted or overwritten, so a fenced, aborted, terminal or tombstoned id can never execute again.

**S6.9 Ambiguous `worktree.prepare`: resolve by the same `lease_id`, never by transport failure or age.**
- *Worker prepare record.* This amends S6.7 step 2. `worktree.prepare` requires `lease.lease_id`. It keeps a durable record under `~/.aoteru/worker-prepare/<lease_id>/`, with the same file roles as the S6.6 spool: `claim.json` (`{kind: "prepare", request}` | `{kind: "fence"}`), `run.json` (`execute` | `abort`), mutable `state.json`, and `result.json`. All decision files go through `decide_once` (S6.11) and none is ever deleted or overwritten.
  - After validation, the verb calls `decide_once(claim.json, {kind: "prepare", request})`. That claim happens **before any git mutation**.
  - The git work runs in a **detached prepare runner** (`python -m src.estate_worker --run-prepare <lease_id>`) spawned with `estate_worker_procs.spawn_detached`. The runner lives in its own cgroup scope on Linux, so S6.12 quiescence covers every `git`/hook descendant. It first calls `decide_once(run.json, {decision: "execute", pid, create_time})`, then calls `create_or_reuse_worktree` → `verify_worktree` → `git_is_clean`, and writes `result.json` + `state.json`:
    - `prepared {path, branch, head_sha, clean}`, with `clean` from `git_is_clean` and never inferred;
    - `prepare_failed {error, stage}` for any exception, including a git timeout.
  - The verb waits for the runner's terminal state up to its `deadline_s`, then returns `ok: true` with the current derived state. A slow prepare therefore answers `preparing`, which the control plane treats as ambiguous.
  - An `ok: false` prepare response can come only from validation **before** the claim: `bad_request`, `identity_*`, `unsupported_protocol`, `unknown_verb`. That makes it a deterministic pre-mutation refusal of that single request.
  - A same-`lease_id` `prepare` that loses the claim returns the existing record (`reused: true`) and never spawns a second runner.
- *Derived prepare state.* As in S6.6:
  - no claim → `unknown`;
  - fence claim, or `run.json` `abort` → `fenced` (nothing ever mutated);
  - claim with no run decision → `preparing`;
  - `execute` with a terminal `state.json` → `prepared`/`prepare_failed`, **but only when `tree_quiescent(runner) is True`** (S6.12). Otherwise it stays `preparing`, because a backgrounded hook or orphaned `git` may still be mutating;
  - `execute`, runner dead, no terminal state, and `tree_quiescent is True` → `prepare_interrupted`: the mutation may be partial, but nothing can still be running;
  - `execute` with the runner dead and `tree_quiescent` `False` or `None` → still `preparing`. Round-0 finding 1: runner death alone never proves quiescence.
- *New verb `worktree.prepare_status`* has payload `{lease_id, fence: bool}` and result `{state: unknown | fenced | preparing | prepared | prepare_failed | prepare_interrupted, quiescent, record}`.
  - `fence: true` with no claim → `decide_once(claim.json, {kind: "fence"})`.
  - `fence: true` with a claim but no run decision → `decide_once(run.json, {decision: "abort", by: "fence"})`, whatever the claim's age. Each has one winner, so a delayed original `prepare` or runner loses and never mutates.
  - Apart from those fence decisions, the verb is read-only.
- *Every branch park prepares through the worker*, the local lab host through `LocalTransport`. This replaces S6.7's "in-process `worktree_ops`" for the local host. One record-and-fence mechanism then covers a lost SSH response and a backend crash during a local prepare alike. The worker verb already wraps the same `worktree_ops` functions.
- *Control-plane classification of the `prepare` call.* This replaces S6.7 step 5.
  - `ok: false` with a worker-side error code (pre-claim refusal) → release the reservation.
  - `ok: true` with `prepare_failed`, or with `prepared` that fails the step-3 evidence check (`clean != true`, branch mismatch, missing `head_sha`) → the prepare positively ended → release the reservation (409 as before).
  - `ok: true` with `prepared` passing step 3 → bind (step 4).
  - Everything else is **ambiguous**, and the reservation stays `preparing`: `worker_unreachable`, `worker_protocol_error`, `placement_mismatch`, timeout, or a lost/unparseable response. The park call returns 409 `{error: "prepare_outcome_unresolved", lease_id, host_id, next_action}`. A bind refused because the reservation changed is not ambiguous, since the reservation is no longer this caller's.
- *Resolution.* New `park_lease_ops.resolve_preparing_reservation(lease_id) -> dict`, with HTTP `POST /api/estate/park/{repo_id}/resolve-prepare` (body `{lease_id, host_id}`, scope `estate:execute`) and CLI `aoteru resolve-prepare <repo_id> --lease <id> --host <id>`.
  1. Read the `preparing` row. With no lock held, call `worktree.prepare_status {lease_id, fence: true}` on its `host_id`.
  2. `preparing` (running, or not proven quiescent), or unreachable/ambiguous → nothing changes. The answer is `prepare_still_running` or `worker_unreachable`.
  3. `fenced`, `prepare_failed`, `prepare_interrupted` or `prepared` → in one S6.5 transaction, re-read the reservation by exact `lease_id`, still `preparing` with the same `host_id`/`branch`, and set it `released`.
  - Resolution **never binds**. Binding stays exclusive to the original park's step 4, because only that caller holds the evidence it asked for. A worktree left prepared becomes clean, unowned evidence that a fresh park reuses through the full S6.7 sequence.
  - `park_repo`/`park_with_worktree` call `resolve_preparing_reservation` automatically, outside the lock, when a competing `preparing` row's `heartbeat_at` is older than `PARK_PREPARE_STALE_SECONDS`. They then retry the reservation once.
- *Reclaimability.* This amends S6.4. `preparing` is **never** `reclaimable` by age. `heartbeat_stale` is still reported as telemetry, and `PARK_PREPARE_STALE_SECONDS = 300` now means only "old enough to probe". The only way out of `preparing`, other than bind, is `resolve_preparing_reservation` after worker-side proof, or a pre-claim/positively-ended release inside the original park call.
- *Blocker.* A `preparing` reservation on a host that never answers again stays protected indefinitely, the same class as B9. See B11.

**S6.10 `finalized` is committed-and-clean. Push is a separate, retryable fact.**
- *Contract.* `worktree_resolution = 'finalized'` means that on the lease branch in that exact worktree, the execution's changes are either:
  - (a) exactly one new commit whose first parent is the execution's `admission_head_sha`; or
  - (b) no commit, when the writer changed nothing, so HEAD is still `admission_head_sha`.

  In both cases the post-finalize `worktree.verify` reports `clean: true`. It does **not** mean pushed. Worktree reuse safety depends only on local state, and the remote is not an input to admission.
- *History bound* (reuses the `git_finalizer.finalize_scoped` idea).
  - Admission step 2 records the verified `head_sha` into the new column `EstateExecution.admission_head_sha`.
  - `start` carries it as `lease.expected_head_sha`. The worker's TOCTOU re-verify refuses before the claim (`authority_denied` → `not_started`) if HEAD differs.
  - `worktree.finalize` carries `expected_head_sha` and refuses (`authority_denied`, the row stays `unresolved`) unless HEAD equals it before commit. After committing, the worker checks that the new commit's first parent equals it.
  - Whether a sandboxed writer can itself commit is *not* relied on (unverified for linked worktrees, whose git dir lies outside the workspace). Any HEAD movement away from `admission_head_sha`, from the writer or anyone else, is treated as history finalize cannot attribute, and finalize refuses it. The only exit is S6.2 recovery, where operator commits are allowed and are recorded as such.
  - Together with single-flight admission on a clean tree, this is what keeps changes from different executions in separate commits.
- *Idempotent, crash-recoverable finalize* (round-0 finding 4, revised after round 1 findings 4 and 8). `worktree.finalize` takes `execution_id` in its payload. The worker keeps a finalize record `~/.aoteru/worker-finalize/<execution_id>/commit.json`, written with `decide_once` **immediately after** a successful commit: `{commit_sha, parent_sha, dirty_paths, at}`. That worker-written record, not the commit message, is the proof of authorship. The commit message still ends with `Aoteru-Execution: <execution_id>`, but only for human traceability, since anyone can forge a trailer. The worker evaluates, in order:
  1. `commit.json` exists → it is authoritative. If HEAD == `commit.json.commit_sha` and the tree is clean → **adopt** (`committed: true, adopted: true`) and attempt the exact-commit push again. Otherwise → `authority_denied` (history moved after the worker's commit).
  2. No `commit.json` and HEAD == `expected_head_sha` → stage the dirty paths, commit, verify the first parent == `expected_head_sha`, then `decide_once(commit.json)`, then attempt the push. If the commit fails, re-evaluate from step 1 once. A concurrent same-execution finalize may have committed.
  3. No `commit.json` and HEAD ≠ `expected_head_sha` → `finalize_ambiguous` with evidence `{head, parent, trailer_matches}`. This can be the crash window between the worker's `git commit` and its `commit.json`, or a commit by someone else, and only an operator can tell which. The row stays `unresolved` with `next_action` = recover.

  Consequences:
  - A lost finalize response, or a control-plane restart between the worker's commit and the S6.5 resolution write, resolves by simply calling `finalize_execution` again (step 1 adoption).
  - The ordinary path for a lost finalize response is to call `finalize_execution` again, before any recovery. Once recovery has closed the execution (S6.12), finalize refuses `execution_closed`. Recovery then records a `commit.json`-proven commit as its `recovery.head_sha` with `finalize_commit: true`, with no refusal and no deadlock (round-9 finding). In the step-3 crash window, recovery is likewise allowed. It records `{head_sha, admission_head_sha, finalize_ambiguous: true}`, and that recorded head is pushable through the governed push below. A worker-created commit that loses its record is therefore never stranded without a supported operation.
  - The resolution write stays conditional on `worktree_resolution = 'unresolved'`, so racing finalizes resolve the row once.
- *Finalize and push git work is unit-scoped too* (round-3 finding 2).
  - `worktree.finalize` and `worktree.push` run their git commands in a transient runner unit `aoteru-finalize-<execution_id>-<n>` / `aoteru-push-<execution_id>-<n>`, where `<n>` comes from a per-execution `decide_once` attempt counter. The verb waits for that unit's `tree_quiescent == True` before reporting any outcome.
  - A deadline that expires first → `finalize_in_progress` / `push_in_progress`. This is ambiguous: the row stays `unresolved` (finalize) or its `push.state` is unchanged (push), and a retry re-evaluates through `commit.json`.
  - Every worker-issued git command in prepare, finalize and push also runs with `-c core.hooksPath=/dev/null -c gc.auto=0 -c maintenance.auto=false`. Hooks and detached auto-gc are not part of the execution contract, and disabling them removes the common sources of backgrounded git descendants. The unit remains the proof, and the flags are only defence in depth.
  - **Every attempt, not just the current one** (round-4 finding 2). Each attempt `<n>` is recorded with `decide_once(attempt-<n>.json, {unit, started_at})` *before* its unit is launched, and the runner records its cgroup in `attempt-<n>.run.json`. The worker reports a finalize outcome only when **all** recorded attempts `1..n` for that execution are quiescent. The same holds for push attempts, which also run from the repo's own path, never the worktree. An attempt whose record exists but whose unit never recorded a cgroup is quiescent only if its `run.json` holds an `abort` decision won by the reporter, the S6.6 fence pattern. Otherwise it is ambiguous. Git's external filter and helper processes are descendants inside the attempt's unit, so this covers them without relying on the hook/auto-gc flags.
  - `finalized` therefore additionally requires the quiescence of every finalize attempt unit. The worker checks it before it answers, and the post-finalize `worktree.verify` re-checks it.
- *Push record.* `finalization_json.push = {state: not_required | pushed | failed, commit_sha, remote: "origin", branch, attempts, last_attempt_at, error?}`.
  - `worktree.finalize` makes one attempt to push **the exact commit**: `git push origin <commit_sha>:refs/heads/<branch>`. It never pushes a branch name and never forces.
  - A rejected non-fast-forward push is followed by `git fetch origin <branch>` and `git merge-base --is-ancestor <commit_sha> FETCH_HEAD`. If the commit is already contained, the state is `pushed`, because a later execution's pushed commit already carries it.
  - A no-commit finalize is `not_required`.
  - The response's `finalized` field means the S6.10 contract. The live `finalized = pushed` conflation (`estate_router.py:1508`) is removed.
- *Governed retry.* New `estate_router.push_finalized_execution(execution_id) -> dict`, with HTTP `POST /api/estate/run/{execution_id}/push` (scope `estate:execute`) and CLI `aoteru push <execution_id>`.
  - Preconditions: the row is `finalized` with `push.state == "failed"`, **or** it is `recovered` with `push.state ∈ {not_attempted, failed}`, and a recorded `recovery.head_sha ≠ admission_head_sha`. In the second case the pushed commit is the recorded recovery head, an explicit operator-invoked operation, never automatic. The worker on the row's own `host_id` must be reachable. No other host has the commit, and there is no cross-host fallback.
  - It calls the new worker verb `worktree.push {execution_id, repo_id, branch, commit_sha}`. The worker resolves the repo's own path, so push works after the worktree is removed. It checks `git cat-file -e <commit_sha>^{commit}` (`not_found` if the commit is absent), then runs the exact-commit push and the containment check above.
  - The result is written with one conditional `UPDATE … WHERE id = :id AND worktree_resolution IN ('finalized', 'recovered')`, merging `push` in `finalization_json`. For a recovered row, `push.commit_sha` = `recovery.head_sha`. Round-2 finding 2: both eligible resolutions record the same canonical push fact. It is not an S6.5 user: it changes no lease or admission authority.
  - It needs **no lease**. Pushing an existing commit by SHA touches neither the worktree nor the local branch, and requiring a lease would strand the commit once the lease is released.
- *Next action.* A `failed` push never blocks admission, ordinary release or recovery. It is always surfaced:
  - `get_estate_execution`, `finalize_execution` and `recover_execution_lease` return `next_action: {"http": "POST /api/estate/run/<id>/push", "cli": "aoteru push <id>"}` in two cases: while `push.state == "failed"`, and for a `recovered` row whose `recovery.head_sha ≠ admission_head_sha` and whose `push.state` is absent or not `pushed`. Recovery itself records `push = {state: "not_attempted", commit_sha: recovery.head_sha}` in that case, so the fact always exists;
  - `release_repo` returns `unpushed_executions: [{execution_id, commit_sha}]` for its lease;
  - `active_leases_summary` shows the same count.

**S6.11 `decide_once`: the single exclusive-decision primitive (worker).** Round-0 finding 5. Every worker-side "exactly one party decides" point uses one helper in `src/estate_worker.py`:
- *Definition.* `decide_once(target: Path, content: dict) -> (won: bool, decided: dict)`.
  1. Write `content` to `<dir>/.<name>.tmp-<uuid>`, `flush` + `fsync`.
  2. `os.link(temp, target)`. `link(2)` on POSIX, and `CreateHardLinkW` via `os.link` on NTFS, both fail with `FileExistsError` when `target` exists, and publish a complete inode, so no reader can ever see a partial decision.
  3. **On a win, `fsync` the containing directory** (`os.open(dir, O_RDONLY | O_DIRECTORY)` + `os.fsync`), and likewise every newly created parent directory. Only then is `won: true` returned, and no caller acts on a decision before that. A crash before the directory fsync therefore loses only a decision nobody has acted on. Any process that might have acted on it died in the same crash.
  4. Unlink the temp file.
  5. If it lost: `fsync` the containing directory **itself**, then read and return the existing `target` (round-2 finding 7). The rule is therefore symmetric: *no process acts on a decision file, whether it won or lost, until it has itself fsynced the directory containing it.* Once that fsync returns, the entry it observed is durable, however long the winner takes to reach its own fsync. Every read of a decision file that feeds a decision (`status`, `start`, fence, prepare, release, finalize adoption) goes through the same `read_decision()` helper. The order is **observe → fsync → act** (round-3 finding 7): read the file, and if it is present, `fsync` its directory *after* observing it, then return it. Decision entries are never removed or replaced, so the post-observation fsync makes durable exactly the entry that was read. An *absent* decision is never acted on directly. Any action taken on absence (fence, claim, abort) is itself a `decide_once`, which carries its own fsync rule.
- *Directory durability* (round-4 finding 1). Every decision directory lives under a fixed per-host anchor, `~/.aoteru`. Its chain of components is made durable **before** any decision is linked into it.
  - `ensure_durable_dir(path)`: for each component from the anchor down, run `mkdir(exist_ok=True)`, then `fsync` its parent. This runs whether or not this caller created the component, because a concurrent creator may not have fsynced yet.
  - `decide_once` calls `ensure_durable_dir(dir)` before step 2, then fsyncs the containing directory as above.
  - `read_decision()` and a losing `decide_once` fsync the containing directory **and every ancestor up to the anchor** after observing the file and before returning it.

  So no decision, and no directory it lives in, is acted on until each directory entry on its path has been fsynced by the actor itself. Directories on the path are never removed, since tombstones keep them.
- *Guarantees.* A decision file is never overwritten, renamed over or deleted. On Linux (ext4/xfs/btrfs), a decision whose directory was fsynced survives a host crash. On Windows, directory fsync is not available through Python. Decision crash-durability there is **unproven**, recorded as B13, and gates the home write lane together with S6.12. Directory `rename` is never used for claims, and directory existence is never a decision.
- *Filesystem requirement.* The spool, prepare roots and every decision file must be on a local filesystem with hard links (ext4/xfs/btrfs, NTFS). On first use per process the worker probes it with a scratch `decide_once` pair: the first link must win, and the second must lose with `FileExistsError`. On failure every write verb (`start` for `codex-write`, `worktree.prepare`, `spool.release`) refuses **before any claim** with `executor_unavailable` "decision filesystem unsupported". `inventory` then reports `codex-write: false`. There is no fallback to `O_EXCL`-then-write or `rename`.
- *Users.* S6.6 `claim.json`/`run.json`, S6.8 `released.json` and fences, and S6.9 prepare `claim.json`/`run.json`.

**S6.12 Tree quiescence: what "cannot still be running" means.** This section was revised after round 1: session scoping alone is insufficient, and the finding is confirmed on lab. Codex's own sandbox runs every command as `bwrap --as-pid-1 --new-session --die-with-parent … --unshare-pid`, which is a `setsid`. Round-0 finding 1 still applies: the death of one pid proves nothing about its descendants.
- *Definition.* A new process-layer function in `src/estate_worker_procs.py`: `tree_quiescent(handle) -> True | False | None`.
- *Linux: cgroup-scoped runners.*
  - Every runner (`_run_spooled`, the S6.9 prepare runner) is launched by `spawn_detached` as a **transient user service**: `systemd-run --user --unit=aoteru-<run|prepare>-<id> --collect --quiet --working-directory=<root> --setenv=HOME=… --setenv=PATH=… -p StandardOutput=append:<log> -p StandardError=append:<log> -- <runner argv>`. The user manager forks the runner into that unit's own cgroup, which contains every descendant, whatever the descendant does with sessions or process groups.
  - Service mode, not `--scope`, because a `--scope` migrates the *calling* process into the user manager's subtree. cgroup v2 lets an unprivileged user migrate a process only when they can write the common ancestor's `cgroup.procs`, and from the backend's `/system.slice/odysseus-aoteru-lab.service` that ancestor is the root cgroup. This is inferred from cgroup v2 rules and not yet observed from inside the service, so the Stage 6 deploy check below verifies it. Service mode needs only user-bus access, which is uid-based.
  - With the unit's default `KillMode=control-group`, leftover descendants are killed when the runner exits, and the unit is collected only once its cgroup is empty. Units live under `user@<uid>.service`, outside the backend's service cgroup, so a backend restart does not kill a runner. Today's `start_new_session` runners are killed with the backend's cgroup.
  - `spawn_detached`'s own pid is `systemd-run`'s, which exits at once, so it is informational only. The **canonical handle** is `run.json`'s `{pid, create_time, cgroup, unit}`, recorded by the runner itself (round-2 finding 3). `status.handle` returns exactly that. The control plane stores it verbatim in `worker_handle_json` when it confirms the execution (step 7), and S6.2 precondition 4 compares against it. Until `run.json` exists there is no handle, and the row keeps its placeholder.
  - The runner writes its own cgroup path from `/proc/self/cgroup` into its `run.json` `execute` decision, and refuses to execute (it decides `abort`) unless that path ends in its own dedicated unit `aoteru-<kind>-<id>.service` (round-3 finding 1: service mode yields `.service`, never `.scope`).
  - `tree_quiescent` is `True` iff that recorded cgroup directory no longer exists, or its `cgroup.events` reports `populated 0`. cgroup v2 `populated` is recursive. The scope is removed only once it is empty, and unit names embed the unique id, so a missing directory cannot be a reused one.
  - It is `False` when the scope is `populated 1`, and `None` when there is no recorded cgroup or `/sys/fs/cgroup` is unreadable.
  - No `/proc` session scan is used as proof.
  - Environment: the worker sets `XDG_RUNTIME_DIR=/run/user/<uid>` for `systemd-run`. The user manager must be running, which needs linger on a headless host (lab: `Linger=yes`).
  - A per-process probe gates every write verb. It runs a transient unit that records its own cgroup and checks that the cgroup is a dedicated `aoteru-probe-<uuid>.service`. `health` reports the result as `write_prerequisites: {decision_fs: bool, runner_units: bool, detail}`, alongside the S6.11 filesystem probe (round-3 finding 3). If it fails, the verb returns `executor_unavailable` before any claim, `inventory` reports `codex-write: false`, and there is no unscoped fallback.
  - Kill: `cancel`/timeout cleanup additionally calls `systemctl --user kill --signal=KILL <unit>`, which reaches every descendant.
  - Quiescence always reads the cgroup path the runner recorded. It never uses `systemctl show … ControlGroup`, which is empty after collection and would otherwise resolve to the root cgroup.
  - Live evidence on lab, 2026-09-23:
    - `codex sandbox -P :workspace` writes inside both a user scope and a transient user service;
    - in a scope, a `setsid`'d grandchild remains a member (`populated 1`, distinct sid), and killing the unit empties and removes the cgroup;
    - in a service, the runner recorded its dedicated `…/app.slice/aoteru-svc-….service` cgroup, and its `setsid` leftover was killed when it exited.
    - **Stage 6 deploy check (operator, before the first real write):** from the running backend, `call_worker(lab, 'health')` must report `write_prerequisites.runner_units: true` and `decision_fs: true`. This proves user-bus access from inside `odysseus-aoteru-lab.service`.
  - A user+PID-namespace wrapper was tried and **rejected**: Codex's `bwrap` probe hangs inside an unprivileged user namespace.
- *Windows.* It returns `None` (unknown) until a job-object implementation is added and live-verified. That implementation would spawn each runner into its own job and report `JOBOBJECT_BASIC_ACCOUNTING_INFORMATION.ActiveProcesses == 0`. Until then no Windows ambiguity that needs quiescence can resolve, and it fails closed. A home write lane needs this anyway, and remains gated by Stage 4 `detached_spawn_verified` + Stage 8.
- *Execution quiescence is an aggregate* (round-5 finding). For an `execution_id`, `execution_quiescent(execution_id)` is `True` only when **every worktree-touching** unit that execution ever launched is quiescent: the S6.6 writer runner, plus every recorded finalize attempt (S6.10). Push attempts are excluded (round-7 finding). They run from the repo's own path, stage and commit nothing, and change no worktree file, index or HEAD. Their own all-attempts quiescence gates only the push outcome report (S6.10). An attempt with no recorded cgroup counts only once fenced, with its abort won. Any `False`/`None` makes the aggregate `False`/`None`. Every consumer below uses the aggregate, never a single unit, so no path (finalize, recovery, release, `interrupted`) can resolve or reuse a worktree while *any* of that execution's git or writer work may still run. For a prepare, the aggregate is its runner unit (S6.9).
- *Execution closure* (round-6 finding). A quiescence snapshot cannot stop a finalize/push call that the control plane authorised earlier but that reaches the worker later. Each execution therefore has a durable worker-side closure fence `closed.json`, written with `decide_once` and never removed.
  - **Attempt side (finalize attempts only):** `decide_once(attempt-<n>.json)` → `read_decision(closed.json)`. If `closed.json` is present → `decide_once(attempt-<n>.run.json, abort)` and refuse with `execution_closed`, before any git command.
  - **Closer side:** `decide_once(closed.json, {by, at})` → `read_decision` of every `attempt-*.json` → aggregate quiescence.
  - Both sides write durably and then read the other's file. Any attempt either sees the closure and aborts, or was recorded before the closer's scan, in which case the aggregate includes it (Dekker ordering over `decide_once`/`read_decision`).
  - `status {close: true}` performs the closer side and returns the aggregate.
  - **Recovery** (S6.2) uses it as its precondition-4 call, after preconditions 1–3. Precondition 5 (the fresh `worktree.verify`, the HEAD read and the `commit.json` check) runs **strictly after** closure has returned aggregate `quiescent: true` (round-8 finding). No finalize attempt can then commit after the HEAD recovery records, because each attempt either aborted on the closure or is proven finished. Recovery enters the S6.5 transaction only with both results in hand, and records exactly the HEAD observed in step 5.
  - A closure that then fails quiescence or a later precondition leaves the execution closed. Finalize can then no longer succeed, so recovery is the only exit, retried once the units are quiescent. That is fail-closed.
  - `spool.release` also performs the closer side first, so every resolved execution is closed to further **finalize** attempts. `worktree.push` never reads `closed.json`. The governed push retry of a `finalized` or `recovered` row (S6.10, U65/U66/I11) therefore stays available after closure, as the no-strand requirement needs.
- *Consumers.* Each consumer treats `None` exactly like `False`.
  - `status` returns the aggregate `quiescent` alongside `process_alive = (quiescent is not True)`, plus `units: [{unit, kind, quiescent}]` as evidence.
  - `interrupted` requires `quiescent == true` (step 8).
  - S6.2 recovery precondition 4 requires a fresh `quiescent == true` in addition to the handle match.
  - `spool.release` requires it (S6.8).
  - Every S6.9 terminal and `prepare_interrupted` state requires it.
- *Residual (B12).* A same-user process *outside* the Codex sandbox that deliberately writes its pid into a different cgroup it owns could leave the scope. The sandbox mounts no `/sys`. This is deliberate evasion by code already running as the operator's own user, not accidental survival, and it is recorded rather than claimed as proven.
...
- `core/database.py`:
  ...
  - add `EstateExecution.admission_head_sha` (String, nullable; S6.10 history bound) and `EstateExecution.spool_released_at` (DateTime, nullable; S6.8 release acknowledgement), both in the same idempotent migration;
...
- `src/estate_router.py`:
  ...
  - `execute_write_via_worker(objective, *, repo_id, host_id, decision_id, wait_timeout=30.0, timeout=1800.0) -> dict`:
    ...
    2. `call_worker(host_id, "worktree.verify", ...)` against that `worktree_path`/`branch`. `ok != true` **or `clean != true`** (S6.1: every admission, including one reopened after finalization, requires a clean worktree) → `authority_denied`/`worktree_not_clean`, no row, no transaction opened. The verified `head_sha` becomes the row's `admission_head_sha` in step 5, and is sent as `lease.expected_head_sha` in `start` (S6.10).
    ...
    8. Start the monitor thread `_observe_worker_execution(execution_id, host_id, timeout)`: poll `status` every 5 s. After each successful observation of a confirmed `accepted`/`running` execution with `process_alive: true`, call `renew_lease_for_execution(execution_id)` (S6.3, exact lease only). No transition in this step changes `worktree_resolution`; outcome and resolution are separate facts.
       ...
       - Worker reachable and terminal (`succeeded`/`failed`/`timed_out`, which the worker reports only once `quiescent: true`; `terminal_pending: true` stays `running`, S6.6) → write the terminal row and `_update_decision_outcome(executed_host_id=host_id, ...)` as before. `worktree_resolution` stays `unresolved`: `succeeded` awaits finalization or recovery, and `failed`/`timed_out` await recovery (S6.1).
  ...
  - `finalize_execution(...)`: keep all DB/lease/worktree drift checks, and make the authority-ordering explicit: before calling worker `worktree.finalize`, re-prove — in the same style of re-check as admission's step 3-4 above, not trusting an earlier check —
    1. the `EstateExecution` row is `succeeded` (an `interrupted` or `lost` execution can never finalize; there is nothing proven-clean to commit), **and** a fresh worker `status` for it reports `quiescent: true` (S6.12; otherwise refused `writer_quiescence_unproven`, nothing changes);
    ...
    3. only then replace the local git block (`:1434-1476`) with `call_worker(execution.host_id, "worktree.finalize", {execution_id, ..., expected_head_sha: row.admission_head_sha})`. The worker performs the requested Git operation only because the control plane has already proven authority — it never decides the lease is valid itself. The branch-drift check moves into the worker verb and the result is echoed. The payload carries `expected_head_sha = row.admission_head_sha`, and the worker enforces the S6.10 history bound and makes one exact-commit push attempt.
    4. then a fresh `worktree.verify` for the same path and branch. If it reports `clean: true`, enter `lease_serialized_transaction(lease_id)`, re-read the row (still `succeeded`/`unresolved`) and the lease (same identity, authoritative), and set `worktree_resolution = 'finalized'` with the finalize result, including `push`, in `finalization_json`. Anything else leaves the row `unresolved` and the lease blocked (S6.1): worker failure, still dirty, drift, `authority_denied`, or `finalize_ambiguous` (with `next_action` = recover). A HEAD ≠ `admission_head_sha` is accepted only as an S6.10 `commit.json`-proven adoption. A `failed` push does not prevent `finalized` (S6.10).
    5. after commit, best-effort `spool.release {resolution: finalized}` (S6.8).
- `src/estate_worker.py`: `decide_once` + its filesystem probe (S6.11); the S6.6 spool layout, claim, runner decision and derived state; the `status` fence and `spool.release` (S6.8); the prepare record, detached `--run-prepare` runner and `worktree.prepare_status` (S6.9); finalize with `expected_head_sha`, trailer adoption and exact-commit push, plus `worktree.push` (S6.10); compaction instead of deletion in the GC (S6.8). `src/estate_worker_protocol.py`: add `worktree.prepare_status`, `spool.release` and `worktree.push` to `VERBS`.
- `src/estate_worker_procs.py`: `tree_quiescent(handle)`. It is cgroup-scoped on Linux and `None` on Windows (S6.12).
- `src/estate_worker_procs.py` `spawn_detached`: the `systemd-run --user --unit=aoteru-<kind>-<id>` transient-service launch, `tree_quiescent` and the runner-unit probe on Linux (S6.12). `_execute_codex_with_sandbox` itself is unchanged.
...
- `routes/estate_routing_routes.py`:
  ...
  - `POST /run/{execution_id}/push` (S6.10, scope `estate:execute`);
  - `POST /park/{repo_id}/resolve-prepare`, JSON body `{lease_id, host_id}` (S6.9, scope `estate:execute`);
  - `POST /park/{repo_id}?host=<host_id>&branch=`:
    - `host` absent or equal to `current_host_id()`: the existing path, except that a `branch` park now goes through `park_with_worktree` with `LocalTransport` worker `worktree.prepare` (S6.9, never in-process `worktree_ops`).
    - Otherwise: require that host to be eligible with `codex-write` in `qualified_executors`, then run `park_with_worktree` with worker `worktree.prepare` as the prepare step. That means reserve (`preparing` row) → prepare with `lease` in the payload → require explicit `clean: true` evidence (never inferred from a successful path/branch verification) → bind to `active`. Failure is classified per S6.9. On a positively known failure (pre-claim refusal, `prepare_failed`, `clean != true`, branch mismatch), the reservation is released and no active ParkLease exists. An ambiguous outcome (unreachable, protocol error, placement mismatch, timeout, lost response, `preparing`) keeps the reservation `preparing` and returns 409 `prepare_outcome_unresolved`. A refused bind leaves the other owner's row untouched. The positively-known refusal has the same 409 shape as the existing `RepoNotClean`/verification-failure path. A concurrent park loses at the reservation and never reaches the worker.
...
- `companion/laptop_client/aoteru.py`:
  ...
  - new `push <execution_id>` (S6.10; POSTs only `/run/{execution_id}/push`) and `resolve-prepare <repo_id> --lease --host` (S6.9; POSTs only `/park/{repo_id}/resolve-prepare`);
...
- No spool directory is ever deleted. A write spool is retained in full until the control plane acknowledges resolution, and a terminal, fenced or tombstoned `execution_id` can never start again (S6.8).
- A `preparing` reservation leaves `preparing` only by bind, by a release on a positively known prepare outcome, or by `resolve_preparing_reservation` after worker-side proof (fence/terminal/dead). Transport failure and age never release or reclaim it (S6.9).
- `finalized` means committed locally, history-bound to `admission_head_sha`, and clean. Push is a separate `finalization_json.push` fact with a governed exact-commit retry that needs no lease, so an unpushed commit always has a supported next action (S6.10).
...
Tests are listed in the §G matrix, items U11a, U13–U36, U29a, U37–U54 and U55–U76, and I4–I11.
...
| U41 | recovery release success: `interrupted` row, fresh fake `status` `quiescent: true` (so `process_alive: false`) with a matching handle, fresh `worktree.verify` `ok`+`clean`. The same fixture with `quiescent: null` or `false` → refused `writer_quiescence_unproven`, nothing changes | in one transaction: row `recovered` with the recovery record, lease `released`. A following park yields a **new** `lease_id`. The old lease cannot be renewed (U43) or used for admission |
...
| U46 | worker same-ID `start` race (S6.6/S6.11): the first `start` is paused by a test hook after winning `claim.json`, before spawn; a second same-ID `start` arrives | second → `{accepted: false, state: "starting", reused: true}`, **not** `execution_failed`. After the first resumes: exactly one `spawn_detached` call and `status` goes `starting → running`. The control plane keeps the row `pending_start` and never writes `failed` in between. Two threads racing `decide_once` on the same target: exactly one `won: true`, both read identical complete content |
...
| U48 | (S6.6, round-0 finding 2) stale `starting` interleavings: (a) observer's abort wins `run.json` → `start_failed`; a runner started afterwards loses `run.json` and exits without calling the writer; (b) runner wins `execute` first, then a stale observer arrives → observer's abort loses, `status` reports `running`, never `start_failed`; (c) a test hook pauses the observer between reading `starting` and deciding, while the runner decides `execute` → still (b). Age alone never yields `start_failed` without winning the decision |
...
| U51 | (S6.9) prepare with a **positively known** failure: pre-claim worker refusal, `prepare_failed`, `prepared` with `clean: false`, branch mismatch | reservation `released`; no `active` or `preparing` lease remains; `eligible_hosts` shows no conflict. Reservation reclaimed before bind → bind refused, and the other owner's row is untouched. A `preparing` row older than `PARK_PREPARE_STALE_SECONDS` is still **not** `reclaimable` (see U56) |
...
| U55 | (S6.9) ambiguous prepare: `worker_unreachable`, `worker_protocol_error`, `placement_mismatch`, transport timeout, lost response | reservation stays `preparing` (not released, not reclaimable); 409 `prepare_outcome_unresolved` with `lease_id`/`next_action`; a competing park is refused `ParkConflict` without any worker call |
| U56 | (S6.9) stale ambiguous reservation (`heartbeat_at` past `PARK_PREPARE_STALE_SECONDS`) → `resolve_preparing_reservation` | `prepare_status {fence: true}` → `preparing` (runner live, or `quiescent` not true) or unreachable: unchanged. `fenced`, `prepare_failed`, `prepare_interrupted` or `prepared`: reservation released in one S6.5 transaction, never bound. A competing `park_repo` triggers this probe itself and then succeeds only after release |
| U57 | (S6.9, worker) prepare record and fence race: the fence is claimed first, then the delayed original `prepare` for the same `lease_id` arrives | the original loses the claim and `create_or_reuse_worktree` is never called. Reverse order: `prepare_status` reports the original's real state. A same-`lease_id` `prepare` after completion returns the record `reused: true` with no second git call. A `preparing` record whose runner is dead reads as `prepare_interrupted` **only with `quiescent: true`**; runner dead with a populated or unknown scope stays `preparing` |
| U58 | (S6.9, worker) post-claim exceptions | a `create_or_reuse_worktree` exception after the claim → `ok: true`, `state: prepare_failed`, persisted. `ok: false` only for pre-claim validation |
| U59 | (S6.9) local branch park goes through `LocalTransport` `worktree.prepare` | the FakeWorker for the lab host receives `worktree.prepare` with the reservation's `lease_id`; `worktree_ops.create_or_reuse_worktree` is never called in-process by the control plane |
| U60 | (S6.8, worker) retention: an unacknowledged `codex-write` spool in each terminal state, aged 30 days | never compacted or deleted; `status` still returns the handle. After `spool.release`, it compacts to a tombstone only once 7 days have passed since release; directory never removed. A self-test spool compacts after 7 days terminal. A `.tmp-` claim older than `STARTING_STALE_SECONDS` is removed |
| U61 | (S6.8, worker) tombstoned/fenced/terminal id replay: `start` with the same `execution_id` | `reused: true`, `accepted: false` (tombstone/fenced) or the terminal state; `spawn_detached` never called |
| U62 | (S6.8) `spool.release` refused for `starting`, or for `accepted`/`running` with `process_alive: true`; idempotent otherwise | control plane: called after each `not_started`/`finalized`/`recovered` commit; failure leaves resolution intact and `spool_released_at` null; reconcile retries it; never sent for legacy rows |
| U63 | (S6.8) placeholder row past `execution_deadline_at` with `status` `unknown` | no `start` re-issued; `status {fence: true}` → `fenced` → `failed` + `not_started`. Before the deadline, `unknown` → same-ID `start` re-issued. A `tombstone` seen for an `unresolved` row → stays `unresolved` with `spool_released_before_resolution` |
| U64 | (S6.10) history bound | admission records `admission_head_sha`; `start` with a moved HEAD → pre-claim `authority_denied` → `not_started`; `finalize` with HEAD ≠ `expected_head_sha` → refused, row stays `unresolved`; successful commit's first parent == `admission_head_sha`; no-change finalize → `finalized`, `push.state = not_required` |
| U65 | (S6.10) push failure then governed retry | finalize with push rejected → `finalized`, `push.state = failed`, `next_action` = push; admission under the same lease still admitted (push never blocks); `release_repo` lists it in `unpushed_executions`. `push_finalized_execution` → worker `worktree.push` on the row's `host_id` only (never another host), with the lease already released → `pushed`. Non-fast-forward with the commit already contained remotely → `pushed` (`contained: true`). Never `--force`; the push refspec is always `<commit_sha>:refs/heads/<branch>` |
| U66 | (S6.10) push preconditions | a row neither `finalized` (with `push.state = failed`) nor `recovered` (with a recorded head ≠ admission head and `push.state ∈ {not_attempted, failed}`) → refused before any worker call; a recovered crash-window head → pushed, and the success recorded on the recovered row (round-2 finding 2); worker unreachable → `worker_unreachable`, push state unchanged; commit absent on the worker → `not_found`, state stays `failed` |
| U67 | (S6.12) quiescence on a real Linux transient user service (skipped with a recorded reason if `systemd-run --user` is unavailable in the test environment): a runner launched through `spawn_detached` starts a `setsid` grandchild and exits | runner dead + grandchild alive → `tree_quiescent` `False` → prepare stays `preparing` (not `prepare_interrupted`), `status.process_alive` true, recovery refused. Runner exit → unit `KillMode=control-group` kills the leftover → cgroup removed → `True`; the grandchild is also killed by `systemctl --user kill`. Recorded cgroup missing from `run.json`, or not the dedicated `aoteru-<kind>-<id>.service`, including the root cgroup, → the runner aborts before executing. `None` (unreadable cgroupfs, Windows) blocks exactly like `False` |
| U68 | (S6.12) runner-unit probe fails (monkeypatched `systemd-run` returning non-zero) | write verbs → `executor_unavailable` before any claim; `inventory.executors.codex-write` false; `spawn_detached` never falls back to an unscoped launch; read-only `execute` unaffected |
| U69 | (S6.8, round-0 finding 3) ambiguous first `start` (transport drop), then a second same-ID `start` refused pre-claim (`authority_denied`, HEAD moved) | the refusal is **not** mapped to `not_started`; the control plane issues `status {fence: true}`. Fence wins → `fenced` → `not_started`; a paused first `start` released afterwards loses `claim.json` and never spawns. If the first had claimed → its real state is used |
| U70a | (S6.11, round-1/2/3 finding 7, round-4 finding 1) `decide_once` durability. Includes a fresh `<execution_id>/` directory created by a winner paused **before** its parent fsync: an observer's `read_decision()` fsyncs the containing directory **and** each ancestor up to `~/.aoteru` before returning (fsync call order asserted) | on a win, `os.fsync` is called on the containing directory fd (and on any new parent) **before** `won: true` is returned. A **loser** — tested with the winner paused by a hook between `link` and its fsync — also fsyncs the directory before returning. `read_decision()` fsyncs **after** observing a present file and before returning it; a reader that observes a file published by a paused winner still returns only after its own fsync. A monkeypatched fsync failure → the call raises, and the caller does not act (no spawn/execute/release/adoption) |
| U70 | (S6.11) decision filesystem probe fails (monkeypatched `os.link` raising `OSError(EPERM)`) | `codex-write` `start`, `worktree.prepare`, `spool.release` → `executor_unavailable` before any claim; `inventory.executors.codex-write` false; no `O_EXCL`/`rename` fallback is attempted |
| U71 | (S6.10, round-0 finding 4, round-1 findings 4/8) finalize commit succeeds, response lost, control plane restarted | second `finalize_execution` (payload carries `execution_id`) → worker adopts HEAD **because `commit.json` matches it** → `finalized` with `push` recorded. A foreign commit with a **forged matching trailer** and the right first parent, but no `commit.json` → `finalize_ambiguous`, never adopted; recovery is then allowed and records the head, and the governed push can push that recorded head. A `commit.json` whose sha ≠ HEAD → `authority_denied`. Recovery with a `commit.json`-proven HEAD (after closure) → `recovered` with `recovery.head_sha` = that commit, `finalize_commit: true`, `push.state = not_attempted`, and `aoteru push <id>` pushes it; no refusal/deadlock (round-9 finding). Two concurrent finalizes → one commit, row resolved once |
| U72 | (S6.8, round-0 finding 5) compaction vs claim concurrency on a real temp dir: compaction runs while a same-ID `start` and a `status {fence: true}` race | `claim.json`/`run.json`/`released.json` never disappear or change; the directory is never observed empty; the id stays unstartable |
| U73 | (S6.6/S6.12, round-2 new finding 1) terminal record but scope still populated (fake `tree_quiescent` False, then True) | `status` → `running` + `terminal_pending: true`; the row stays `running`; `finalize_execution` refused `writer_quiescence_unproven` before any worker `worktree.finalize`; once quiescent → `succeeded`, finalize proceeds |
| U74 | (S6.10/S6.12, round-3/4 finding 2, round-5 finding) finalize/push unit quiescence. Recovery variant: a finalize attempt has committed (tree clean) but is still populated without `commit.json` → `recover_execution_lease` refused `writer_quiescence_unproven` (aggregate false) and the lease stays active; after the attempt unit empties → recovery allowed | fake finalize unit stays populated past the deadline → `finalize_in_progress`, row `unresolved`, no resolution write; quiescent → outcome reported. Overlap: attempt 1 writes `commit.json` and stays populated, attempt 2 adopts → still `finalize_in_progress` until attempt 1's unit is quiescent; an attempt record with no cgroup is fenced (abort won) before it counts as quiescent. Argv assertion: every worker git call carries `core.hooksPath=/dev/null`, `gc.auto=0` and `maintenance.auto=false`; the finalize/push git runs under an `aoteru-finalize-*`/`aoteru-push-*` unit |
| U75 | (S6.12, round-6/8 findings) execution closure vs a stale finalize. Ordering assertion: recovery's `status {close: true}` call precedes its `worktree.verify` call, and a finalize attempt committing while closure waits leads recovery to record the **post-commit** HEAD (with `finalize_commit: true` once `commit.json` exists), never the pre-commit HEAD | `finalize_execution` paused after its authority/quiescence checks, before the worker call; recovery runs `status {close: true}` (closure written, aggregate quiescent) and releases the lease; a fresh park + E2 dirties the worktree; the paused finalize resumes → worker records the attempt, reads `closed.json`, aborts, refuses `execution_closed`, and runs no git command. Reverse interleaving (the attempt is recorded before closure) → the closer's aggregate includes it and recovery waits. Randomised thread interleavings on a real temp dir never produce a stale-finalize git call **during or after** a successful recovery, and a successful recovery always records exactly the final HEAD. Clarified at the Stage 6 gate (round 8): an attempt that won its run decision *before* the closure may still run git, but the closure waits for it (aggregate not quiescent), which is exactly the preceding requirement that recovery record the post-commit HEAD |
| U76 | (S6.12, round-7 finding) push after closure | `finalized` row with `push.state = failed`, `spool.release` done (`closed.json` present) → `aoteru push <id>` still runs `worktree.push` (carrying `execution_id`) and records `pushed`; the same for a `recovered` crash-window head. A finalize attempt after closure → `execution_closed`. A populated push unit never blocks recovery or `spool.release` |
...
| I11 | (S6.8/S6.9/S6.10) through a real `LocalTransport` worker and a real temp git repo with a bare `origin`: (a) a park whose first `prepare` response is dropped by a transport shim → reservation `preparing` → `resolve-prepare` fences/reads the record → released → fresh park succeeds and reuses the clean worktree; (b) a noop-sleep execution finalized with no changes → `spool.release` → tombstone after a clock-advanced GC → a replayed same-ID `start` does not spawn; (c) a real commit finalized while `origin` rejects the push (pre-receive hook) → `push.state = failed` → the hook is removed → `aoteru push <id>` → `pushed`, with `origin/<branch>` equal to the exact commit |
...
| B9 | (narrowed by S6.8: a `lost` placeholder row whose host returns is fenced and resolved `not_started`) A `lost` execution on a host that never becomes reachable again keeps its lease protected indefinitely (S6.2 deliberately refuses `lost`; there is no force flag). Clearing it needs the host back, or a separate governed, out-of-band DB decision. Stage 6 does not automate either | operator decision | only that repo's writes, and only after such a loss |
...
| B11 | (S6.9) A `preparing` reservation whose host never answers `worktree.prepare_status` again stays protected indefinitely (age alone never reclaims it). Same class and same operator resolution as B9 | operator decision | only that repo's parks, and only after such a loss |
| B12 | (S6.12) Linux quiescence is cgroup-scoped (systemd user scope). Residual: a same-user process outside the Codex sandbox that deliberately migrates itself to another cgroup it owns. Windows quiescence is `None` until job objects are implemented and live-verified | follow-up / Stage 8 | Windows recovery/release proofs; nothing on lab beyond the documented residual |
| B13 | (S6.11) Windows `decide_once` crash-durability (no directory fsync from Python) is unproven | Stage 8 home write qualification | the home write lane only |
