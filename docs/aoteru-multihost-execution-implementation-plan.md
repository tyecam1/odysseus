...
- `src/estate_router.py`:
  ...
  - `execute_write_via_worker(objective, *, repo_id, host_id, decision_id, wait_timeout=30.0, timeout=1800.0) -> dict`:
    ...
    2. `call_worker(host_id, "worktree.verify", ...)` against that `worktree_path`/`branch`. Failure → `authority_denied`, no row, no transaction opened.
    3. **Enter one control-plane DB transaction** (pre-Stage-6 review finding, item 3 — atomic admission). Inside it, re-read the row for the **exact `lease_id`** from step 1 (not a fresh "current lease for repo/host" lookup, which could silently pick up a *different* lease issued after step 1) and require, all under the same transaction:
       - still `active` and non-stale under the execution-aware rules in the Invariants section below;
    ...
    4. Still inside that same transaction, `_in_flight_execution_for_lease` under that exact `lease_id`. Every non-reusable state blocks a new admission: `accepted`, `running`, an unresolved `pending_start` (step 6's uncertain-`start` sub-state), `lost`, **and `interrupted`** (item 4 below — a positively-dead writer may have partially modified the worktree, so `interrupted` is terminal as a process state but not automatically reusable as a write-authority state). A blocking row → `{ok: False, error_code: "lease_has_unresolved_lost_execution"}` (or an analogous `lease_has_unresolved_execution` code covering the `interrupted` case), `execution_id` of the blocking row; a genuinely reusable accepted/running row (whose `start` is still being resolved per step 6) → reuse, `dispatch: "reused_in_flight"`. This is what keeps admission single-flight under one lease — no second row, no second store, and (per item 4) no second writer dispatched into a worktree an interrupted execution may have left dirty. Recovery from `interrupted` is explicitly operator-driven, not automatic: `interrupted → inspect/clean/recover worktree → explicit release → fresh park / fresh lease_id → new execution`.
    5. Still inside the same transaction: `_create_estate_execution` (accepted), with `worker_handle_json` set to a non-null dispatch-state placeholder (e.g. `{"dispatch_state": "pending_start"}`), **not** `NULL` — the row is durably marked as a real, in-flight Stage-6 execution from the moment it exists, before `start` is even attempted (this is also what step 4's "unresolved `pending_start`" blocking state refers to). `execution_deadline_at` (item 5 below) is set here too, from `timeout`, so the value driving `lost` reconciliation is durable from the moment the row exists, not a process-local argument. Commit.
    6. Only after that transaction commits does the control plane call `call_worker(host_id, "start", ...)`. **A transport/protocol failure or lost response here is not proof nothing started** (pre-Stage-6 review finding: an earlier pass already corrected the plan's prior "row failed, because nothing started" claim — this restates the corrected state machine precisely, item 7 below). The worker may have already created the spool and spawned the detached writer before the response was lost; `start`'s own idempotency per `execution_id` (same `execution_id` in a retried `start` → existing handle, `reused: true`, never a second spawn) exists specifically so this can be resolved safely. The explicit state machine:
       ...
       - retried `start` reports `reused: true` → the execution is **confirmed**; proceed to step 7 (transition to `running`) — a successful same-ID retry is **never** evidence for `failed`, regardless of how the first attempt's response was lost;
       ...
       - a **deterministic pre-spawn refusal** from the worker (e.g. `authority_denied`, `bad_request` — the worker positively refused to even attempt the spawn) → this alone may transition the row to `failed`;
    ...
    8. Start the monitor thread `_observe_worker_execution(execution_id, host_id, timeout)`: poll `status` every 5 s.
       ...
       - Worker unreachable, or otherwise not currently observable → keep polling; do not guess. With no successful observation past the row's persisted `execution_deadline_at + 120 s` → `lost` (outcome undetermined, not "process dead"). Using the persisted deadline rather than the `timeout` argument this call happened to be started with is exactly item 5's point: after a control-plane restart, `_observe_worker_execution` may not even be the thread that resumes watching this row, so the bound must be reconstructable from the row alone.
       - No redispatch occurs merely because a row became `lost` or `interrupted` — those are observations, not admission decisions; a new admission still goes through step 4's single-flight check (which, for `interrupted`, stays blocked until the operator's explicit recovery, not merely until observation).
  ...
  - `reconcile_stale_estate_executions(db, EstateExecution)`:
    ...
    - rows past their persisted `execution_deadline_at + 120 s` (item 5 — never a process-local `timeout` argument, so this survives a control-plane restart with the exact same bound) with no successful observation → `lost`;
    - `lost` rows → try `status` when the worker becomes reachable again, and reconcile to the truthful spool/process state this reveals (`interrupted` if the runner is confirmed dead, a terminal state if the spool already has one, or back to `running` if it is genuinely still alive) — `lost` was never itself the true state, only the bound on how long "undetermined" was tolerated. No redispatch follows from this reconciliation alone; an `interrupted` outcome stays blocked from lease reuse until the operator's explicit recovery (item 4), same as an `interrupted` row reached directly by the monitor thread;
  ...
  - `finalize_execution(...)`: keep all DB/lease/worktree drift checks, and make the authority-ordering explicit (item 8): before calling worker `worktree.finalize`, re-prove — in the same style of re-check as admission's step 3-4 above, not trusting an earlier check —
...
- `src/park_lease_ops.py` (contract only — Stage 6 does not add a second lock/lease/lifecycle store; this couples the *existing* ParkLease stale/release rules to EstateExecution state, item 2):
  - stale-reclaim eligibility (today: heartbeat age alone) additionally requires that no `EstateExecution` row under this `lease_id` is in `accepted`, `running`, an unresolved `pending_start`, or `lost` — a running or unresolved remote writer must never lose its only write authority merely because the lease's own heartbeat aged out (the write itself, at `timeout=1800.0`, can already run as long as the current stale threshold). `lost` specifically must **not** by itself permit reclaim — the outcome being undetermined is exactly the case where reclaiming the lease out from under a possibly-still-running writer would be least safe. An `interrupted` execution, per item 4, also keeps the lease blocked from ordinary reclaim/release, but through the explicit-recovery path, not the stale-heartbeat path;
  - ordinary `release_repo`/`heartbeat` calls get the same check: `release` while the lease's active `EstateExecution` is `accepted`/`running`/`lost` is refused (the caller must resolve the execution first — e.g. `wait` for it, or, once it is genuinely terminal, proceed normally); `interrupted` requires the explicit recovery path in item 4 (`interrupted → inspect/clean/recover worktree → explicit release → fresh park / fresh lease_id`), which is itself a form of release the operator performs only after confirming the worktree, not a blanket refusal forever;
  - the control plane — never the worker — is what renews the ParkLease heartbeat while it is actively supervising a confirmed `accepted`/`running` execution (the same monitor thread from step 8 above, or an equivalent periodic renewal tied to that supervision); the worker has no DB access and never heartbeats or mutates ParkLease directly, consistent with the DB-free-worker invariant below.
- `routes/estate_routing_routes.py`:
  - `GET /run/{execution_id}?wait=<int 0..60>`;
  - `POST /park/{repo_id}?host=<host_id>&branch=`: if `host` is absent or equals `current_host_id()`, run the existing path. Otherwise require that host to be eligible with `codex-write` in `qualified_executors`, call worker `worktree.prepare`, then require its response to carry explicit cleanliness evidence for the prepared/reused worktree (item 6 — `worktree.prepare`'s response, or a following `repo.probe`-style verification, must state `clean: true`; cleanliness is never inferred from a successful path/branch verification alone, the same standard the local park path already holds itself to). `clean != true` → park fails closed, no ParkLease row, same shape of refusal as the existing `RepoNotClean`/verification-failure → 409 path. Only once clean is confirmed does `park_repo(repo_id, host, path, branch)` run.
  - `/park/{repo_id}/heartbeat` and `/release` accept `?host=` (DB-only), subject to the execution-aware release rules in the `park_lease_ops.py` bullet above.
...
- Admission under one lease stays single-flight, including `lost`, `interrupted` (blocked until explicit operator recovery, not merely until observation) and an unresolved `start` (pre-Stage-6 review finding: an uncertain `start` response reuses the same single-flight check as `lost`, never opening a second admission window).
- Lease re-validation and EstateExecution admission are one atomic control-plane transaction (item 3): the exact `lease_id` checked before remote `worktree.verify` is re-read and re-validated — active, non-stale, held by `route.host`, correct scope/worktree/branch — inside the same transaction that inserts the new row, so a lease released or reassigned in between never results in a row being created or `start` being issued.
- A ParkLease backing an `accepted`/`running`/unresolved-`pending_start`/`lost` execution is protected from automatic stale reclaim and ordinary release (item 2); the control plane, never the worker, renews that lease's heartbeat while actively supervising it.
- A persisted `execution_deadline_at` (item 5), set when the row is created, is what `lost` reconciliation is computed from — never a process-local `timeout` argument — so a control-plane restart reconstructs the exact same bound.
- Remote `worktree.prepare` requires explicit worker-attested cleanliness evidence before a ParkLease is created (item 6); cleanliness is never inferred from path/branch verification alone.
- The control plane proves ParkLease authority (`_lease_authority`, then the step 3-4 atomic re-validation above) before issuing any remote `worktree.verify`, `start`, or `worktree.finalize` call — `worktree.finalize` re-proves it a second time, immediately before the call, per item 8. The worker remains DB-free and never decides lease validity, routing, or lifecycle authority — the lease payload sent to the worker is evidence/instruction from the already-authenticated control plane, not an independent worker-side lease authority.

Tests are listed in the §G matrix, items U11a and U13–U36, and I4–I8.
...
| U29 | atomic admission race: lease valid at the initial `_lease_authority` check, then released/reassigned to a different host before the admission transaction opens | the re-read inside the admission transaction (step 3-4) sees the changed/absent lease and refuses; **no** `EstateExecution` row is created; **no** worker `start` call occurs — proves the early check alone is never relied on |
| U30 | running execution (`accepted`/`running`), lease heartbeat artificially aged past the ordinary stale threshold | stale-reclaim refused — the execution being actively supervised protects the lease regardless of heartbeat age |
...
| U32 | `release_repo`/`heartbeat` called while the lease's execution is `accepted`/`running`/`lost` | refused; `interrupted` requires the explicit item-4 recovery path (inspect/clean/recover → release → fresh park) rather than an ordinary release, and is refused by the ordinary path too |
| U33 | execution positively observed active (`status` succeeds, `accepted`/`running`) during supervision | the control plane's own heartbeat-renewal call on the lease is invoked (fake DB/lease layer asserts a renewal call happened); the worker fake never receives any lease/heartbeat-shaped call — it has no DB access and cannot mutate ParkLease |
| U34 | dispatch under lease with an `interrupted` row | refused (same error shape family as U21's `lease_has_unresolved_lost_execution`); no new row, no `start` call — proves item 4's "not automatically reusable" distinct from U19's "positively observed" outcome |
...
| I6 | Sept 3 sequence: dispatch E, duplicate while running → same E (single-flight, per the Invariants section), observe via status only to terminal, new dispatch → new E with `dispatch: new`. Also covers the ambiguous-`start` case from step 6 above through a real transport: kill the transport after the worker has actually spawned but before its `start` response reaches the control plane → row is not marked `failed`; a retried `start` with the same `execution_id` observes `reused: true` and the row proceeds to `running` off the one real spawn — the successful retry itself is asserted as never having been treated as evidence for `failed` at any point in between |
| I7 | remote-host park via `?host=` with FakeWorker `worktree.prepare` reporting a clean worktree → ParkLease row `host_id=desktop-in7o23d`; heartbeat/release with `?host=` |
| I8 | remote-host park via `?host=` with FakeWorker `worktree.prepare` reporting `clean: false` (item 6) → refused before any ParkLease row is created — same 409/`RepoNotClean`-shaped failure the local park path already produces for a dirty worktree, not a silent success |
