...
def execute_codex_write_durable(objective: str, *, repo_id: str, host_id: str,
    ...
    def _run() -> None:
        ...
        # routes here after Stage 6). It predates worktree resolution, so it
        # keeps its pre-Stage-6 admission semantics by closing its own
        # terminal row as `legacy_closed` -- but ONLY when the process tree
        # is known gone. A timeout whose cleanup left survivors
        # (`cleanup_incomplete`) stays `unresolved`, so the lease stays
        # protected (Stage 6a/6b adjudication finding 1).
        _update_estate_execution(
            ...
            worktree_resolution="unresolved" if result.get("cleanup_incomplete") else "legacy_closed",
        )
        if decision_id:
